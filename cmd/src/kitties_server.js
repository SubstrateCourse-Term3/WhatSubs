        ...
        if (event.section === 'kitties') {
            ...
            switch (event.method) {
                case 'Created': {
                    ...
                    let kitty = (await api.query.kitties.kitties(kittyEvent[1])).toJSON();
                    if (kitty !== null) {
                        await db.runAsync("insert or replace into t_kitties(owner_addr, kitty_index, kitty_dna, lifespan, birthday) values (?,?,?,?,?)",
                            kittyEvent[0], kittyEvent[1], kitty.dna, kitty.lifespan, kitty.birthday);
                    }
                    break;
                }
                case 'Died': {
                    ...
                    await Promise.all([
                        db.runAsync("delete from t_kitties where kitty_index=?", kittyEvent[1]),
                        db.runAsync("delete from t_kitty_prices where kitty_index=?", kittyEvent[1]),
                    ]);
                    break;
                }
                case 'Transferred': {
