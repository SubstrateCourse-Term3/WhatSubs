...
async function main(db) {
    ...
    for (let i = 0; i < count; i++) {
        let [kitty, owner, price, db_kitty, db_price] = await Promise.all([
            ...
            db.getAsync("Select owner_addr, kitty_dna, lifespan, birthday from t_kitties where kitty_index=?", i),
        ...
        assert.strictEqual(db_kitty.kitty_dna, kitty.dna);
        assert.strictEqual(db_kitty.lifespan, kitty.lifespan);
        assert.strictEqual(db_kitty.birthday, kitty.birthday);
        ...
        console.log(`${i}: dna: ${kitty.dna}, owner: ${owner}, price: ${price}`);
    }
}
